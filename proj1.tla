...
define {
   canCommit ==    \A rm \in RM  : rmState[rm] \in {"prepared"} 
                 \/ \E rmc \in RM : rmState[rmc] \in {"committed"}
...
RC:        rmState[self] := "committed";}
        or {
           await rmState[self]="working" \/  (rmState[self]="prepared" /\ tmState="abort");
...
***************************************************************************)
\* BEGIN TRANSLATION - the hash of the PCal code: PCal-7b871645a1a28648f2e5d54eb62f90d6
...
RS(self) == /\ pc[self] = "RS"
            /\ IF pre[self] \notin {"committed", "aborted"}
                  THEN /\ \/ /\ rmState[self] = "working"
                          ...
                          \/ /\ rmState[self]="working" \/  (rmState[self]="prepared" /\ tmState="abort")
...
RManager(self) == RS(self) \/ RC(self) \/ RA(self) \/ RT(self)
...
\* END TRANSLATION - the hash of the generated TLA code (remove to silence divergence warnings): TLA-04a78953a516c3da628162839b1f8062
...
\* Last modified Fri Jun 12 21:23:42 EDT 2020 by anjalibachani
